...
# ==============================================================================
# 2. 나스닥 100 / 코스피 400 / 코스닥 200 안전 캐싱 로더
# ==============================================================================
...
# ==============================================================================
# 3. yfinance Rate Limit 방어용 캐싱 함수
# ==============================================================================
    ...
    try:
        ...
        if not info or "currentPrice" not in info and "regularMarketPrice" not in info:
            ...
            return {
                ...
                "returnOnEquity": 0, "operatingMargins": 0,
                "totalRevenue": "N/A", "netIncomeToCommon": "N/A"
            }
        return info
    except Exception:
        # Rate limit 발생 시 최소한의 fast_info 대체
        try:
            fast = stock.fast_info
            return {
                "currentPrice": fast.last_price,
...
with col_custom:
    if target_ticker == "CUSTOM":
        target_ticker = st.text_input("직접 티커 입력 (예: NVDA, 005930.KS)", value="NVDA").strip().upper()
    else:
        st.text_input("연동 티커", value=target_ticker, disabled=True)

# ==============================================================================
# 6. 실시간 팩트 데이터 추출 & 4대 마스터 탭
# ==============================================================================
if target_ticker and target_ticker != "CUSTOM":
    ...
    with tab4:
        ...
        c1, c2 = st.columns(2)
        with c1:
            my_buy_price = st.number_input("내 매수 단가", value=float(current_price if current_price else 100.0))
        with c2:
            my_peak_price = st.number_input("매수 이후 형성된 최고가", value=float(max(current_price if current_price else 100.0, my_buy_price)))
